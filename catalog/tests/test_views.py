...
class AuthorCreateViewTest(TestCase):
    ...
    def test_form_date_of_death_initially_set_to_expected_date(self):
        ...
        self.assertEqual(
            response.context['form'].initial['date_of_death'], initial_date_of_death)

    def test_redirects_to_detail_view_on_success(self):
        login = self.client.login(
            username='testuser2', password='2HJ1vRV0Z&3iD')

        valid_date_birth = datetime.date.today() - datetime.timedelta(weeks=70)
        valid_date_death = datetime.date.today() - datetime.timedelta(weeks=20)
        response = self.client.post(reverse('author-create'), {
            'first_name': "Bertrand",
            'last_name': "Russell",
            'date_of_birth': valid_date_birth,
            'date_of_death': valid_date_death
        })
